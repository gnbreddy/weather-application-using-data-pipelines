...
import IndianCitiesWeatherGrid from '../components/IndianCitiesWeatherGrid';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchWeatherData = async (location = 'San Francisco') => {
      try {
    ...
    // Try to get user location first, then fetch weather
    const initializeWeather = async () => {
      try {
        const location = await getUserLocation(false); // Don't show toasts on initial load to prevent spam
        if (location && location.source !== 'fallback') {
          setUserLocation(location);
          fetchWeatherData(location.city);
          console.log('✅ Using user location:', location.city);
        } else {
          console.log('⚠️ Using fallback location: San Francisco');
          fetchWeatherData('San Francisco');
        }
      } catch (error) {
        console.log('❌ Location failed, using San Francisco:', error.message);
        fetchWeatherData('San Francisco');
      }
    };
    ...
    const interval = setInterval(() => {
      // Get fresh location data each time instead of relying on stale closure
      const getCurrentLocationCity = () => {
        const cached = document.cookie
          .split(';')
          .find(row => row.trim().startsWith('weather_app_location='));
        
        if (cached) {
          try {
            const locationData = JSON.parse(decodeURIComponent(cached.split('=')[1]));
            return locationData.location?.city || 'San Francisco';
          } catch (e) {
      ...
      const locationToUse = getCurrentLocationCity();
      fetchWeatherData(locationToUse);
    }, 5 * 60 * 1000);
  ...
  const recentAlerts = [
    { id: 1, type: 'warning', message: 'High temperature anomaly detected in San Francisco', time: '2 min ago' },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Indian Cities Weather Grid */}
      <IndianCitiesWeatherGrid />
